...
def run(platform: str, account: str = "default", since: str | None = None, days: int | None = None, dry_run: bool = False) -> dict:
    ...
    if platform == "all":
        ...
        return {
            ...
            "status": "ok",
            "records": [
                metrics.fetch(name, account=account, since=since, days=effective_days, dry_run=dry_run, config={"account": account})
                for name in platforms
            ],
        }
    return metrics.fetch(platform, account=account, since=since, days=effective_days, dry_run=dry_run, config={"account": account})
